...
func TestIDPartsExtraction(t *testing.T) {
	for i, v := range IDs {
		t.Run(fmt.Sprintf("Test%d", i), func(t *testing.T) {
			if got, want := Time(v.id), time.Unix(v.timestamp, 0); got != want {
				t.Errorf("Time() = %v, want %v", got, want)
			}
			if got, want := Machine(v.id), v.machine; !bytes.Equal(got, want) {
				t.Errorf("Machine() = %v, want %v", got, want)
			}
			if got, want := PID(v.id), v.pid; got != want {
				t.Errorf("Pid() = %v, want %v", got, want)
			}
			if got, want := Counter(v.id), v.counter; got != want {
...
func TestNew(t *testing.T) {
	...
	for i := 1; i < 10; i++ {
		...
		secs := Time(id).Sub(Time(prevID)).Seconds()
		if secs < 0 || secs > 30 {
			t.Error("wrong timestamp in generated ID")
		}
		// Check that machine ids are the same
		if !bytes.Equal(Machine(id), Machine(prevID)) {
			t.Error("machine ID not equal")
		}
		// Check that pids are the same
		if PID(id) != PID(prevID) {
			t.Error("pid not equal")
		}
		// Test for proper increment
		if got, want := int(Counter(id)-Counter(prevID)), 1; got != want {
...
func TestIDJSONUnmarshalingError(t *testing.T) {
	...
	err = json.Unmarshal([]byte(`{"ID":0}`), &v)
	if err != ErrInvalidID {
		t.Errorf("json.Unmarshal() err=%v, want %v", err, ErrInvalidID)
	}
	err = json.Unmarshal([]byte(`{"ID":false}`), &v)
	if err != ErrInvalidID {
		t.Errorf("json.Unmarshal() err=%v, want %v", err, ErrInvalidID)
	}
}
...
func TestZero(t *testing.T) {
	got := ID{}
	if want := Zero(); !reflect.DeepEqual(got, want) {
		t.Error("Zero() not equal ID{}")
	}
}
